...
private fun exportToCsv() {
  ...
  //  val file = File(getExternalFilesDir(null), fileName)
    ...
    file.printWriter().use { out ->
        out.println("Name,Email,Mobile,Address,Password")
        fetchedList.forEach {
            out.println("${it.ownerName},${it.email},${it.mobile},${it.address},${it.password}")
        }
    }
    //Toast.makeText(this, "CSV saved: ${file.absolutePath}", Toast.LENGTH_LONG).show()
    Toast.makeText(
        this,
 ...
 */
   /*
    private fun exportToPdf() {
        ...
        fetchedList.forEach {
            document.add(
                Paragraph("${it.ownerName} | ${it.email},${it.mobile},${it.address},${it.password}")
            )
        }
        document.close()
        Toast.makeText(
